		...
		<div class=" mt-4">
			<div class="dropdown mb-3 pt-3 col-9 mx-auto">
			  ...
			  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
			    <?php echo '<a class="dropdown-item" href="phones.php?id='. $_GET['id'] .'&brend=Honor">Honor</a>' ?>
			...
			<?php 
			    ...
			    $brend = $_GET['brend'];
				$b = mysqli_query($link, "SELECT * FROM ". $brend ." ");
			...
			<div class="row mb-3 border py-2">
				<div class="col-2 text-center">
					<?php 
						echo '<a style="text-decoration: none; color: black" href="tel.php?id_tel='. $product1['id'] .'&id='. $_GET['id'] .'"><img class="w-75" src ="images/' . $product1['img'] . '"></a>';
					?>
				</div>
				<div class="col-7">
					<?php 
						echo '<a style="text-decoration: none; color: black" href="tel.php?id_tel='. $product1['id'] .'&id='. $_GET['id'] .'"><h5>' . ' ' . $product1['name'] . '</h5>';
						echo '<p>' . $product1['cpu'] . ', ' . $product1['ram'] . ', ' . $product1['resolution'] .  ', ' . $product1['memory'] . '</p></a>';
					?>
				</div>
				<div class="col-3">
					<div class="row">
						<form action="insert_korzina.php" method="POST">
							<?php
								echo '<input name ="count" type ="" value = "">';
								echo '<input name ="type" type ="hidden" value = "' . $product1['type'] . '">';
								echo '<input name ="user_id" type ="hidden" value = "' . $_GET['id'] . '">';
								echo '<input name ="product_id" type ="hidden" value = "' . $product1['id'] . '">';
								echo '<h4>' . $product1['price'] . ' рублей </h4>';
							?>
							<button class="btn btn-warning ml-3">В корзину</button>
						</form>
					</div>
